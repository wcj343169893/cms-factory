...
<div class="m950">
	<div class="m_l640 f_l">
		...
		<?php if(is_array($maincat)) { foreach($maincat as $i => $c) { ?>
			...
			<div class="box_body">
				<div class="thumb"><?php echo tag("moduleid=$moduleid&catid=".$c['catid']."&condition=status=3 and thumb<>''&order=".$MOD['order']."&length=20&width=120&height=90&pagesize=".$MOD['page_icat']."&cols=4&template=thumb-table");?></div>
			</div>
		<?php } } ?>
		<?php } ?>
	</div>
	...
	<div class="m_r f_l">
		...
		<div class="box_body">
		<div class="rank_list"><?php echo tag("moduleid=$moduleid&condition=status=3 and addtime>$today_endtime-60*86400&order=hits desc&pagesize=10", -2);?></div>
		</div>
	</div>
</div>
